...
pub async fn view(id: String, output_format: &str) -> Result<()> {
    ...
    let finding = if let Ok(id_num) = id.parse::<i64>() {
        FindingQueries::get_by_id(db.pool(), id_num)
            .await
            .map_err(|e| miette::miette!("{}", e))?
    } else {
        FindingQueries::get_by_finding_id(db.pool(), &id)
            .await
            .map_err(|e| miette::miette!("{}", e))?
    };
...
pub async fn update(id: String, status: String, note: Option<String>) -> Result<()> {
    ...
    let finding_id = if let Ok(id_num) = id.parse::<i64>() {
        ...
        let finding = FindingQueries::get_by_finding_id(db.pool(), &id)
            .await
            .map_err(|e| miette::miette!("{}", e))?
    ...
    FindingQueries::update(db.pool(), finding_id, &update)
        .await
        .map_err(|e| miette::miette!("{}", e))?;
...
pub async fn delete(id: String, confirm: bool) -> Result<()> {
    ...
    FindingQueries::delete(db.pool(), finding_id)
        .await
        .map_err(|e| miette::miette!("{}", e))?;
...
pub async fn export(output: String, format: String) -> Result<()> {
    ...
    let findings = FindingQueries::list(db.pool(), None, None, None, None)
        .await
        .map_err(|e| miette::miette!("{}", e))?;
    ...
    let content = match format.to_lowercase().as_str() {
        ...
        _ => {
            return Err(miette::miette!(
                "不支持的格式: {}. 可用格式: json, sarif, markdown",
                format
            ))
        }
    };
...
fn export_markdown(findings: &[crate::database::Finding]) -> Result<String> {
    ...
    // 按严重程度分组
    let by_severity =
        |sev: &str| -> Vec<_> { findings.iter().filter(|f| f.severity == sev).collect() };
