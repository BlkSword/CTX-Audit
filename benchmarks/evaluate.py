...
def load_audit_log(audit_log_path: str, verdict_mode: str = "true_positive") -> List[dict]:
    """读取 Agent 审计日志，按 verdict 过滤后转为 findings 格式。

    verdict_mode:
      - true_positive: 只保留 verdict=true_positive（高精确模式）
      - actionable:    保留 true_positive + needs_review（人工仍需复核，但会进入报告）
      - all:           保留全部 verdict（包括 false_positive，用于审计算法调试）
    """
    ...
    findings = []
    for e in entries:
        verdict = e.get("verdict", "needs_review")
        if verdict_mode == "true_positive" and verdict != "true_positive":
            continue
        if verdict_mode == "actionable" and verdict not in ("true_positive", "needs_review"):
            continue
        # 转换为与 scan findings 一致的字段
        findings.append({
            "file_path": e.get("file_path", ""),
            "line_start": e.get("line", 0),
            "line_end": e.get("line", 0),
            "vuln_type": e.get("vulnerability_type", ""),
            "severity": e.get("severity", "medium"),
            "description": e.get("reasoning", ""),
            "verdict": verdict,
            "confidence": e.get("confidence"),
            "investigation_steps": e.get("investigation_steps"),
            "specialist_result": e.get("specialist_result"),
...
def main():
    ...
    parser.add_argument("--findings", help="Path to CTX-Audit JSON findings")
    parser.add_argument("--audit-log", help="Path to CTX-Audit audit_log.json (Agent 模式)")
    parser.add_argument("--verdict-mode", default="true_positive", choices=["true_positive", "actionable", "all"], help="Agent verdict 过滤模式")
    parser.add_argument("--mode", default="default", help="Scan mode label (default/taint/deep/agent)")
    ...
    if not args.findings and not args.audit_log:
        parser.error("--findings 或 --audit-log 至少指定一个")

    cwe_filter = set(int(x.strip()) for x in args.cwe_filter.split(",") if x.strip().isdigit()) if args.cwe_filter else None

    if args.audit_log:
        findings = load_audit_log(args.audit_log, verdict_mode=args.verdict_mode)
    else:
